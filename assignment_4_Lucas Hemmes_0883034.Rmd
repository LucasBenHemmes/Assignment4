...
title: | 
  ...
  | 2022-2023
author: |
  | Lucas Hemmes Github: LucasBenHemmes
  |l.b.hemmes@students.uu.nl
  | http://www.github.com/LucasBenHemmes
date: April 2023
urlcolor: purple
linkcolor: purple
output: 
...
GrowthSW <- mutate(GrowthSW, treat = revolutions > 0) #False is no revolution!

datasummary(treat * (growth + rgdp60) ~ mean + median + sd + min + max, data = GrowthSW)
 
 #with some themes, the table will not appear!
...
The mean and median growth of countries that did not have a revolution is higher than the countries that did.
...
P = 0.06871 > 0.05 alternative hypothesis holds true true difference in means between group FALSE and group TRUE is not equal to 0
...
rgdp60 is the value of gdp per capita in 1960 in 1960 US dollars. The purpose of adding this, is adding a starting point of the GDP in this model.
...
education is the main driver, in model 4 it explains 31.8% of growth. this is when education was added, this made the most difference out of all 4 variables. It also has 3 stars, which means its statistically significant.
...
list(model1, model2, model3, model4) |>
  ...
  row_spec(4, color = 'white') %>%
      row_spec(4, background = 'red') %>%
   kableExtra::save_kable(file = "table.docx")
